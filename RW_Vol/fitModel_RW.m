function r = fitModel_RW(responses, inputs, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% - hgf_config
% - hgf_ar1_config
r.c_prc = tapas_rw_binary_config;%hgf_binary3l_reward_social_config_diacon;
...
%                                      hgf_binary3l_reward_social_config)
r.c_obs = tapas_softmax_binary_config;%softmax_reward_social_config_diacon;      
...
disp('Model quality:');
disp(['    LME (more is better): ' num2str(r.F)])
disp(['    AIC (less is better): ' num2str(r.optim.AIC)])
disp(['    BIC (less is better): ' num2str(r.optim.BIC)])
disp(' ')
disp(['    AIC and BIC are approximations to -2*LME = ' num2str(-2*r.F) '.'])
disp(' ')
...
if any(isinf(H(:))) || any(isnan(H(:))) || any(eig(H)<=0)
    if isfield(r.optim, 'T')
        ...
        disp('Warning: Cannot calculate Sigma and F(=LME) because the Hessian is not positive definite.')    
    end
else
    % Parameter covariance
    Sigma = inv(H);
...
% Calculation of AIC and BIC from logLl added by Jen Cook 2 July 2015
AIC  = 2*(-1*logLl) +2*d;
BIC  = 2*(-1*logLl) +d*log(ndp);
% AIC  = 2*negLl +2*d;
...
r.optim.Sigma = Sigma;
r.optim.Corr  = Corr;
r.F = F; %= the same as LME 
r.logLl = logLl;
r.optim.negLl = negLl; %inserted by Egbert
r.optim.negLj = negLj;%inserted by Egbert
r.optim.accu  = accu;%inserted by Egbert
r.optim.comp  = comp;%inserted by Egbert
r.optim.AIC   = AIC;%inserted by Egbert
r.optim.BIC   = BIC;%inserted by Egbert

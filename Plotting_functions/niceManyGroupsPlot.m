function a = niceManyGroupsPlot(data, cond, varargin)
% Written by Rebecca Lawson, May 2016.
% Edited by Rebecca Lawson, Aug 2018.
% Hacked by Jen Cook, Nov 2018, to display 1SEM (not 1.96 SEM)
...
set([H.semPtch], 'FaceColor',colors([p.Results.col1+1],:));
set([H.semPtch], 'EdgeColor', [0,0,0]);
set([H.semPtch], 'FaceAlpha',0.5);
set([H.sdPtch], 'FaceColor',colors(p.Results.col1,:));
set([H.sdPtch], 'FaceAlpha',0.25);
set([H.sdPtch], 'EdgeColor', colors(p.Results.col1,:));
set([H.sdPtch], 'EdgeAlpha', 0);
set([H.mu], 'Color', [0 0 0]);
...
set([H2.semPtch], 'FaceColor',colors([p.Results.col2+1],:));
set([H2.semPtch], 'EdgeColor', [0,0,0]);
set([H2.semPtch], 'FaceAlpha',0.5);
set([H2.sdPtch], 'FaceColor',colors(p.Results.col2,:));
set([H2.sdPtch], 'FaceAlpha',0.25);
set([H2.sdPtch], 'EdgeColor', colors(p.Results.col2,:));
set([H2.sdPtch], 'EdgeAlpha', 0);
set([H2.mu], 'Color', [0 0 0]);
...
legend(a, p.Results.gplab, 'Location','best'); % 'best');
legend('boxoff')
...
%% get the means for each condition to fit the line
 ...
 Ya = [y{1}(1,1) y{2}(1,1) y{3}(1,1)];
 
 x2=get([H2.mu],'XData'); y=get([H2.mu],'YData');
  Yc = [y{1}(1,1) y{2}(1,1) y{3}(1,1)];
...
legend(a, p.Results.gplab, 'Location', 'north');%'best');
legend('boxoff')

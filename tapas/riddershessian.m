...
% any later version). For further details, see the file COPYING or <http://www.gnu.org/licenses/>.
    ...
    for i = 1:n
        ...
        fxi_ar = @(xi) fxi(f,x,i,xi); %AR changed for later matlab versions
        
        % Calculate derivative
        [hessf(i,i), err(i,i)] = tapas_riddersdiff2(fxi_ar,x(i),options); %AR changed for later matlab versions
    end
    ...
    for i = 2:n % rows
        for j = 1:i-1 % columns
            ...
            fxixj_ar = @(xixj) fxixj(f,x,i,j,xixj); %AR changed for later matlab versions
        
            % Calculate cross-derivative
            [hessf(i,j), err(i,j)] = tapas_riddersdiffcross(fxixj_ar,[x(i),x(j)],options); %AR changed for later matlab versions
            hessf(j,i) = hessf(i,j);
            err(j,i)   = err(i,j);
        end
    end
